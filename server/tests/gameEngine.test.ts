...
import { seatingOrderFromDealer } from '../src/games/hazari/deck.js';
...
describe('getPublicState().playersConfirmedArrangement', () => {
  it('starts empty and fills in one at a time as each player confirms', () => {
    const game = new HaazariGame('ROOM1', PLAYERS, 'P1');
    dealFixtureRound(game);
    expect(game.getPublicState().playersConfirmedArrangement).toEqual([]);

    game.confirmArrangement('P1', ARRANGED_SETS.P1);
    expect(game.getPublicState().playersConfirmedArrangement).toEqual(['P1']);

    game.confirmArrangement('P3', ARRANGED_SETS.P3);
    const after2 = game.getPublicState().playersConfirmedArrangement;
    expect(after2).toContain('P1');
    expect(after2).toContain('P3');
    expect(after2).not.toContain('P2');
    expect(after2).not.toContain('P4');
    expect(after2.length).toBe(2);
  });

  it('never exposes the actual cards - only playerIds', () => {
    const game = new HaazariGame('ROOM1', PLAYERS, 'P1');
    dealFixtureRound(game);
    game.confirmArrangement('P1', ARRANGED_SETS.P1);
    const confirmed = game.getPublicState().playersConfirmedArrangement;
    // Every entry must be a bare playerId string, not an object with cards.
    for (const entry of confirmed) {
      expect(typeof entry).toBe('string');
    }
  });

  it('reaches all 4 once everyone has confirmed, and resets on the next round', () => {
    const game = new HaazariGame('ROOM1', PLAYERS, 'P1');
    dealFixtureRound(game);
    for (const p of PLAYERS) game.confirmArrangement(p, ARRANGED_SETS[p]);
    expect(game.getPublicState().playersConfirmedArrangement.sort()).toEqual([...PLAYERS].sort());

    const order = game.getCurrentPlayOrder();
    for (const pid of order) game.playSet(pid);
    // Still within round 1 (more sets to play) - arrangement stays confirmed for everyone.
    expect(game.getPublicState().playersConfirmedArrangement.length).toBe(4);
  });
});
...
describe('requestDismissal - NO_SEQUENCE claim BEFORE confirming (the proposedSets fix)', () => {
  ...
  const noSequenceHand = [
    { rank: 'A' as const, suit: 'SPADES' as const, id: 'SPADES_A' },
    { rank: 'A' as const, suit: 'HEARTS' as const, id: 'HEARTS_A' },
    { rank: '3' as const, suit: 'SPADES' as const, id: 'SPADES_3' },
    { rank: '3' as const, suit: 'HEARTS' as const, id: 'HEARTS_3' },
    { rank: '5' as const, suit: 'SPADES' as const, id: 'SPADES_5' },
    { rank: '5' as const, suit: 'HEARTS' as const, id: 'HEARTS_5' },
    { rank: '7' as const, suit: 'SPADES' as const, id: 'SPADES_7' },
    { rank: '7' as const, suit: 'HEARTS' as const, id: 'HEARTS_7' },
    { rank: '9' as const, suit: 'SPADES' as const, id: 'SPADES_9' },
    { rank: '9' as const, suit: 'HEARTS' as const, id: 'HEARTS_9' },
    { rank: 'J' as const, suit: 'SPADES' as const, id: 'SPADES_J' },
    { rank: 'J' as const, suit: 'HEARTS' as const, id: 'HEARTS_J' },
  ...
  // Any 3+3+3+4 split of the hand above works for this test.
  const draftSets: [typeof noSequenceHand, typeof noSequenceHand, typeof noSequenceHand, typeof noSequenceHand] = [
    [noSequenceHand[0], noSequenceHand[1], noSequenceHand[2]],
    [noSequenceHand[3], noSequenceHand[4], noSequenceHand[5]],
    [noSequenceHand[6], noSequenceHand[7], noSequenceHand[8]],
  ...
  it('THE FIX: succeeds when the draft arrangement is submitted alongside the claim', () => {
    ...
    const dealerBefore = game.dealerId;
    const outcome = game.requestDismissal('P1', 'NO_SEQUENCE', draftSets);
    expect(outcome.ok).toBe(true);
    expect(game.state).toBe('DISMISSED_ROUND');
    for (const p of PLAYERS) expect(game.cumulativeScores[p]).toBe(0);
    expect(game.dealerId).not.toBe(dealerBefore);
  ...
  it('rejects a proposed arrangement that does not match the cards actually dealt (integrity check)', () => {
    const game = new HaazariGame('ROOM1', PLAYERS, 'P1');
    dealFixtureRound(game);
    (game as any).hands['P1'] = noSequenceHand;

    const fakeCard = { rank: 'Q' as const, suit: 'CLUBS' as const, id: 'CLUBS_Q' }; // not in P1's hand
    const tamperedSets: any = [
      [fakeCard, noSequenceHand[1], noSequenceHand[2]],
      [noSequenceHand[3], noSequenceHand[4], noSequenceHand[5]],
      [noSequenceHand[6], noSequenceHand[7], noSequenceHand[8]],
      [noSequenceHand[9], noSequenceHand[10], noSequenceHand[11], noSequenceHand[12]],
    ];
    const outcome = game.requestDismissal('P1', 'NO_SEQUENCE', tamperedSets);
    expect(outcome.ok).toBe(false);
  });

  it('an ALREADY-CONFIRMED arrangement still takes priority over a proposed one', () => {
    const game = new HaazariGame('ROOM1', PLAYERS, 'P1');
    dealFixtureRound(game);
    (game as any).hands['P1'] = noSequenceHand;
    // Directly inject a confirmed arrangement (bypassing confirmArrangement's
    // stricter strongest->weakest ordering check, which isn't the concern
    // of this test - only that a confirmed record takes priority).
    (game as any).arrangements['P1'] = { playerId: 'P1', sets: draftSets, confirmed: true };

    // Even passing a nonsense/garbage proposedSets should be ignored, since
    // the already-confirmed arrangement is used instead.
    const outcome = game.requestDismissal('P1', 'NO_SEQUENCE', undefined);
    expect(outcome.ok).toBe(true);
  });

  it('does NOT allow claiming dismissal for a hand that genuinely has a sequence, even with a crafted proposal', () => {
    const game = new HaazariGame('ROOM1', PLAYERS, 'P1');
    dealFixtureRound(game);
    // P1's real fixture hand (from ARRANGED_SETS) has a real sequence in it.
    const realSets = ARRANGED_SETS['P1'];
    const outcome = game.requestDismissal('P1', 'NO_SEQUENCE', realSets);
    expect(outcome.ok).toBe(false);
  });
});
