...
export class HaazariGame {
  ...
  requestDismissal(
    playerId: PlayerId,
    claimedReason: DismissalRequest['claimedReason'],
    proposedSets?: [Card[], Card[], Card[], Card[]]
    ...
    const hand = this.hands[playerId];
    let arrangement = this.arrangements[playerId]?.sets;
    if (!arrangement && proposedSets) {
      if (!setsExactlyPartitionHand(hand, proposedSets)) {
        return { ok: false, error: 'Your proposed arrangement does not match your dealt hand.' };
      }
      arrangement = proposedSets;
    }
  ...
  getPublicState() {
    ...
    return {
      ...
      // Who has confirmed their hand arrangement so far (ARRANGING_HANDS /
      // WAITING_FOR_HAND_CONFIRMATION only) - just the fact of confirmation,
      // never the actual cards, so waiting players can see who they're
      // still waiting on without any privacy leak.
      playersConfirmedArrangement: this.playersClockwise.filter((pid) => this.arrangements[pid]?.confirmed),
...
/**
 * Lightweight check (deliberately NOT the same as validatePlayerArrangement):
 * confirms `sets` uses exactly the cards in `hand` - correct sizes (3,3,3,4),
 * no duplicates, no foreign cards, nothing missing - WITHOUT requiring the
 * strongest-to-weakest ordering rule. Used to sanity-check a *proposed* (not
 * yet confirmed) arrangement submitted alongside a dismissal claim: ordering
 * validity is irrelevant to whether a sequence exists in the hand, so
 * requiring it here would let a legitimately-hopeless hand get wrongly
 * rejected just because the player's draft wasn't fully valid yet.
 */
function setsExactlyPartitionHand(hand: Card[], sets: [Card[], Card[], Card[], Card[]]): boolean {
  const sizesOk = sets[0].length === 3 && sets[1].length === 3 && sets[2].length === 3 && sets[3].length === 4;
  if (!sizesOk) return false;
  const handIds = hand.map((c) => c.id).sort();
  const setIds = sets
    .flat()
    .map((c) => c.id)
    .sort();
  if (handIds.length !== setIds.length) return false;
  return handIds.every((id, i) => id === setIds[i]);
}
